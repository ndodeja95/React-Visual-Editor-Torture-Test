...
const catalog: CatalogItem[] = [
  ...
  { id: 'DOC-002', title: 'Sectioning landmarks', category: 'Document & sectioning', pattern: 'Nested section tags', expected: 'Selection only', filters: ['selection-only'], instruction: 'Click the nested section labels. Verify the editor can isolate each landmark rather than selecting the wrapper.' },
  ...
  { id: 'GROUP-001', title: 'Preformatted source', category: 'Headings & grouping', pattern: 'pre / code block', expected: 'Direct edit', filters: ['direct-edit'], instruction: 'Select the preformatted block and edit its content without collapsing whitespace or line breaks.' },
                ...
                <section key={category} id={`section-${category}`} className="scroll-mt-24" data-testid={`section-${category}`}>
                  ...
                  <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
                    {items.map((item) => {
                      ...
                      if (item.id === 'DOC-002') return <TestCard key={item.id} {...item}><div className="grid gap-2 text-xs"><section className="rounded border border-[hsl(var(--border))] p-2" data-testid="DOC-002-target-section"><strong>Section A</strong><section className="mt-2 rounded bg-[hsl(var(--accent)/.25)] p-2" data-testid="DOC-002-target-nested">Nested section B</section></section></div></TestCard>;
                      ...
                      if (item.id === 'GROUP-001') return <TestCard key={item.id} {...item}><pre className="overflow-auto rounded border border-[hsl(var(--border))] bg-[hsl(var(--sidebar))] p-3 text-left text-xs leading-5 text-[hsl(var(--sidebar-foreground))]" data-testid="GROUP-001-target-pre"><code>{`const source = "preserve";\nreturn source + "\\nline breaks";`}</code></pre></TestCard>;
