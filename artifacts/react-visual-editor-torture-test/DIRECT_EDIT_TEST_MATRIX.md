...
| HEAD-003 | Headings & grouping | hgroup, h3, p | Heading group | Select group and child nodes | Direct edit | One heading or summary | Select heading and supporting summary | Group boundary and child mapping remain clear | Not tested. | |
| GROUP-001 | Headings & grouping | pre, code | Preformatted source | Select preformatted block | Direct edit | One code block | Edit content while preserving whitespace | Line breaks and spacing remain intact | Not tested. | |
...
| FORM-004 | Forms | input, datalist, option | input list / datalist | Select input, datalist, option | Selection only | One suggestion source | Focus input and open suggestions | Suggestions work and source nodes are distinguishable | Not tested. | |
| FORM-005 | Forms | textarea | Controlled multiline value | Select textarea | Direct edit | One textarea | Edit multiple lines | Textarea remains controlled and line count updates | Not tested. | |
| FORM-006 | Forms | select, optgroup, option | Grouped select options | Select select and option hierarchy | Selection only | One select subtree | Open select and inspect groups/options | Select remains usable and hierarchy is documented | Not tested. | |
...
| SRC-002 | Nonvisual & source-only | noscript (escaped) | Escaped noscript markup | Source map only | Agent fallback expected | Source documentation only | Inspect escaped example | No live noscript node is rendered | Not tested. | |
...
| REACT-003 | React patterns | slot | Slot / fallback content | Select host and slot fallback | Agent fallback expected | Slot boundary | Select host and fallback content | Slot semantics are recorded without errors | Not tested. | |
